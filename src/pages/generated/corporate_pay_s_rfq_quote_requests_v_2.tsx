...
export default function CorporatePayRFQQuoteRequestsV2() {
    ...
    setRfqs((prev) =>
      prev.map((r) => {
        ...
        const invited = r.invited.map((i) => {
          if (i.status === "Not invited") return { ...i, status: "Invited", invitedAt: now, lastSeenAt: now };
          return i;
        });
  ...
  const markVendorViewed = (rfqId: string, vendorId: string) => {
    ...
    setRfqs((prev) =>
      prev.map((r) => {
        if (r.id !== rfqId) return r;
        const invited = r.invited.map((i) => (i.vendorId === vendorId ? { ...i, status: i.status === "Invited" ? "Viewed" : i.status, lastSeenAt: now } : i));
        return { ...r, invited };
      })
    );
  ...
  const submitQuote = () => {
    ...
    setRfqs((prev) =>
      prev.map((r) => {
        ...
        const invited = r.invited.map((i) => (i.vendorId === q.vendorId ? { ...i, status: "Quoted", lastSeenAt: now } : i));
    ...
    setRfqs((prev) =>
      prev.map((r) => {
        ...
        const milestones = r.po.milestones.map((m) =>
          m.id === milestoneId ? { ...m, status: m.status === "Paid" ? "Planned" : "Paid" } : m
        );
