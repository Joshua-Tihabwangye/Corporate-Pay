...
type SettingsItem = {
  ...
  health: Health;
  quickActions: Array<{ id: string; label: string; kind: "open" | "copy" | "action"; hint?: string }>;
};
  ...
  return (
    <div className="relative grid place-items-center">
      <svg width={size} height={size} className="-rotate-90">
        ...
        <circle cx={size / 2} cy={size / 2} r={r} stroke={EVZ.green} strokeWidth={stroke} fill="none" strokeDasharray={`${dash} ${c}`} strokeLinecap="round" />
      </svg>
...
export default function CorporatePayAdminSettingsHubV2() {
  ...
  const items = useMemo<SettingsItem[]>(() => {
    const mk = (partial: Omit<SettingsItem, "lastUpdatedAt"> & { lastUpdatedDaysAgo: number }): SettingsItem => ({
      ...partial,
    ...
    return [
      mk({
        ...
        description: "Company profile, entities, billing contacts, branding, invoice groups, and support toggle.",
      ...
      mk({
        ...
        description: "Enable service modules and marketplaces including MyLiveDealz. Includes Other slots for expansion.",
        category: "Organization",
      ...
      mk({
        ...
        description: "Invite employees, set group and cost center defaults, bulk import, and offboarding.",
        category: "Access",
        path: "/settings/users",
        tier: "Core",
      ...
      mk({
        ...
        description: "Create Groups, cost centers, project tags, and chargeback rules.",
        category: "Access",
      ...
      mk({
        ...
        description: "Custom roles, module permissions, two-admin model controls, delegation, and simulation.",
        category: "Access",
        path: "/settings/roles",
        tier: "Core",
        roles: ["Org Admin"],
      ...
      mk({
        id: "policy_builder",
        title: "Policies",
        description: "Policy builder for rides, services, purchases and marketplace restrictions with inheritance.",
        category: "Controls",
      ...
      mk({
        ...
        description: "Create approval flows per module and marketplace including RFQs and high value assets.",
        category: "Controls",
      ...
      mk({
        ...
        description: "Org, group, user, module, and marketplace budgets with forecasting and exceptions.",
        category: "Finance",
      ...
      mk({
        ...
        description: "Invoice frequency, invoice groups, tax, templates, and billing simulations.",
        category: "Finance",
      ...
      mk({
        ...
        description: "Spend reports, approval performance, exports, custom report builder, and schedules.",
        category: "Reporting",
        path: "/reports",
        tier: "Core",
      ...
      mk({
        ...
        description: "EV usage metrics, charging utilization, emissions estimates, and ESG template exports.",
        category: "Reporting",
        path: "/reports/esg",
        tier: "Premium",
        roles: ["Org Admin", "Finance"],
      ...
      mk({
        ...
        description: "ERP exports, secure API keys, scopes, webhooks, sandbox mode, and inspector.",
      ...
      mk({
        ...
        description: "Audit logs, MFA policies, retention, support mode gating, dual control, forensic exports.",
        category: "Security",
      ...
      mk({
        ...
        description: "Embedded support role, auditable sessions, scoped tools, recording policy, and status messages.",
        category: "Support",
  ...
  const categories: Category[] = ["All", "Organization", "Access", "Controls", "Finance", "Integrations", "Security", "Support", "Reporting"];
  const [category, setCategory] = useState<Category>("All");
  const [query, setQuery] = useState("");
  const [viewAs, setViewAs] = useState<ActorRole>("Org Admin");
  const [pinned, setPinned] = useState<string[]>(() => ["org_setup", "security", "integrations"]);
  ...
  const canAccessSelected = useMemo(() => selected.roles.includes(viewAs), [selected.roles, viewAs]);
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .filter((i) => {
        if (!q) return true;
        const blob = `${i.title} ${i.description} ${i.category} ${i.tags.join(" ")} ${i.path} ${i.tier}`.toLowerCase();
        return blob.includes(q);
      })
      .slice()
      .sort((a, b) => {
        // pinned first, then by health severity, then recently updated
  ...
  const goLiveReadiness = useMemo(() => {
    // Demo readiness derived from a few items
    const keyIds = ["org_setup", "modules_marketplaces", "billing_invoice_groups", "security", "integrations"];
  ...
  // Right rail: recent changes (demo)
  const recentChanges = useMemo(() => {
    const list = items
      ...
      .map((i) => ({
        ...
        at: i.lastUpdatedAt,
        title: i.title,
        note: i.health === "Healthy" ? "Updated configuration" : i.health === "Needs attention" ? "Requires review" : "At risk limit",
        path: i.path,
        health: i.health,
      }));
    return list;
        ...
        <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_30px_90px_rgba(2,8,23,0.12)]">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs text-slate-500">One place to manage every CorporatePay setting. Use search, filters, and role-based access.</div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 rounded-3xl border border-slate-200 bg-white p-4 md:grid-cols-12">
              <div className="md:col-span-7">
                ...
                <div className="mt-2 flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2.5">
                  ...
                  />
                  {query ? (
                    <button className="rounded-xl p-1 text-slate-500 hover:bg-slate-100" onClick={() => setQuery("")} aria-label="Clear">
                      <X className="h-4 w-4" />
                    </button>
                  ) : null}
                </div>
                  ...
                  {categories.map((c) => (
                    <button
                      ...
                      className={cn(
                        ...
                        category === c ? "text-white ring-emerald-600" : "bg-white text-slate-700 ring-slate-200 hover:bg-slate-50"
                      )}
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            selectedId === it.id ? "border-emerald-300 ring-4 ring-emerald-100" : "border-slate-200"
                          )}
                          onClick={() => {
                            setSelectedId(it.id);
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex min-w-0 items-start gap-3">
                              <div className={cn("grid h-11 w-11 shrink-0 place-items-center rounded-2xl text-white", locked ? "bg-slate-400" : "")} style={locked ? undefined : { background: EVZ.green }}>
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="mt-1 text-xs text-slate-500">{it.category} • Updated {timeAgo(it.lastUpdatedAt)}</div>
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <button
                                ...
                                className={cn(
                                  "rounded-2xl p-2",
                                  pinnedOn ? "bg-amber-50 text-amber-900 ring-1 ring-amber-200" : "text-slate-500 hover:bg-slate-100"
                                )}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  togglePin(it.id);
              ...
              <div className="lg:col-span-4 space-y-4">
                <Section
                  ...
                  right={<Pill label={canAccessSelected ? "Accessible" : "Locked"} tone={canAccessSelected ? "good" : "warn"} />}
                >
                  <div className="rounded-3xl border border-slate-200 bg-white p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-xs text-slate-500">{selected.category} • Updated {timeAgo(selected.lastUpdatedAt)}</div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      <Button
                        variant={canAccessSelected ? "primary" : "outline"}
                        onClick={() => openItem(selected)}
                      >
                        <ChevronRight className="h-4 w-4" />
                        {canAccessSelected ? "Open" : "Request access"}
                      </Button>
                      <Button variant="outline" onClick={() => copyLink(selected.path)}>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          toast({ title: pinned.includes(selected.id) ? "Unpinned" : "Pinned", message: selected.title, kind: "info" });
                        }}
                      >
                ...
                <Section title="Go-live readiness" subtitle="Quick snapshot of key configuration." right={<Pill label={goLiveReadiness >= 80 ? "On track" : "Needs work"} tone={goLiveReadiness >= 80 ? "good" : "warn"} />}>
                  <div className="flex items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-white p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-xs text-slate-500">Org setup, modules, billing, security, integrations</div>
                      </div>
                    </div>
                    <Button
                      variant="outline"
                      ...
                      .map((it) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex items-start gap-3">
                                <div className={cn("grid h-10 w-10 place-items-center rounded-2xl text-white", locked ? "bg-slate-400" : "")} style={locked ? undefined : { background: EVZ.green }}>
                                  <Icon className="h-5 w-5" />
                                </div>
                                <div>
                              ...
                              <div className="flex items-center gap-2">
                                {locked ? <Lock className="h-4 w-4 text-slate-400" /> : <ChevronRight className="h-5 w-5 text-slate-400" />}
                              </div>
                            </div>
                          </button>
                        );
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 text-xs text-slate-500">{timeAgo(c.at)} • {c.note}</div>
                          </div>
                          <ChevronRight className="h-5 w-5 text-slate-400" />
                        </div>
                      </button>
          ...
          <footer className="border-t border-slate-200 bg-white/60 py-6">
            <div className="mx-auto max-w-[1400px] px-4 text-xs text-slate-500 md:px-6">
              Admin Settings Hub v2: single umbrella page linking to all CorporatePay settings. Includes search, category filters, pinned shortcuts, role-gated access requests, and a readiness snapshot.
            </div>
          </footer>
        </div>
      </div>
