...
export default function CorporatePayGlobalSearchCommandCenterV2() {
	...
	const [rfqDraft, setRfqDraft] = useState({
		...
		neededBy: "2026-02-01",
		description:
			"Need a company vehicle. Include warranty and delivery timeline.",
		attachments: true,
	});
		...
		return {
			...
			invoices: [
				...
				},
				{
					...
					amount: 6800000,
					group: "Operations",
					due: now + 5 * 24 * 60 * 60 * 1000,
				},
			],
			trips: [
				{
					id: "TR-0442",
					route: "Office → Airport",
					rider: "John S.",
					amount: 82000,
					time: now - 45 * 60 * 1000,
				},
			...
			rfqs: [
				...
				{
					id: "RFQ-402",
					title: "Bulk EV charging credits",
					estValue: 8000000,
					status: "Pending",
					module: "EVs & Charging",
				},
	...
	const results = useMemo(() => {
		...
		const match = (it: SearchItem) => {
			...
			if (!q) return true;
			const blob =
				`${it.scope} ${it.id} ${it.title} ${it.subtitle} ${it.meta} ${(it.tags || []).join(" ")}`.toLowerCase();
	...
	const [saved, setSaved] = useState<SavedSearch[]>(() => [
		{
			...
			query: "overdue",
			scopes: ["Invoices", "Payments" as any].filter(Boolean) as any,
		},
		{
			id: "SS-002",
			role: "Approver",
	...
	useEffect(() => {
		setSaved((prev) =>
			prev.map((s) => ({
				...s,
				scopes: s.scopes.filter((x) =>
					scopesAll.includes(x as any),
				) as ScopeKey[],
			})),
		);
	...
	const saveSearch = () => {
		...
		if (!n) {
			toast({
				title: "Missing name",
				message: "Enter a name for the saved search.",
				kind: "warn",
			});
			return;
		}
	...
	const applySaved = (s: SavedSearch) => {
		...
		toast({
			title: "Applied",
			message: `Applied saved search: ${s.name}`,
			kind: "success",
		});
	};
	...
	const performAction = () => {
		...
		if (!requireReasonOk) {
			toast({
				...
				message: "Add a clear reason and confirm acknowledgement.",
				kind: "warn",
			});
			return;
		}
		...
		if (actionKind === "rfq") {
			toast({
				title: "RFQ created",
				message: `${rfqDraft.title} • ${formatUGX(rfqDraft.estValue)}`,
				kind: "success",
			});
		}
		...
		if (actionKind === "budget") {
			toast({
				title: "Budget issued",
				message: `${budgetDraft.group}: ${formatUGX(budgetDraft.amount)} (${budgetDraft.period})`,
				kind: "success",
			});
		}
		...
		if (actionKind === "freeze") {
			toast({
				title: "CorporatePay frozen",
				message: `${freezeDraft.mode} • Notifications: ${freezeDraft.notifyUsers ? "On" : "Off"}`,
				kind: "warn",
			});
		}
		...
		if (actionKind === "export") {
			toast({
				...
				message: `${exportDraft.month} • ${exportDraft.format}`,
				kind: "success",
			});
		}
	...
	const filteredCommands = useMemo(() => {
		...
		const list = !q
			? commands
			: commands.filter((c) =>
					`${c.label} ${c.description} ${c.group} ${c.shortcut}`
						.toLowerCase()
						.includes(q),
				);
		// group order: Quick actions, Navigate, Tools
		const order = (g: Command["group"]) =>
			g === "Quick actions" ? 0 : g === "Navigate" ? 1 : 2;
	...
	const resetScopes = () => {
		...
		toast({
			title: "Scopes reset",
			message: "Searching across all entities.",
			kind: "info",
		});
	};
				...
				footer={
					<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
						<div className="text-xs text-slate-500">
							Tip: use Ctrl+K (or Cmd+K) to open. Use / to focus
							search.
						</div>
						<Button
							variant="outline"
					...
					<div className="border-b border-slate-200 bg-white px-4 py-4 md:px-6">
						<div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
							<div className="flex items-start gap-3">
								...
								<div>
									...
									<div className="mt-1 text-xs text-slate-500">
										Search users, invoices, vendors, RFQs,
										approvals and more. Run audited admin
										actions.
									</div>
							...
							<div className="flex flex-wrap items-center gap-2">
								...
								>
									<Command className="h-4 w-4" /> Open
									commands
								</Button>
								...
								<div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
									<div className="flex items-start justify-between gap-3">
										<div>
											...
											<div className="mt-1 text-xs text-slate-500">
												Premium: saved searches per
												role.
											</div>
										</div>
										<Pill
											label={`${savedForRole.length}`}
											tone="neutral"
										/>
									</div>
							...
							<div className="lg:col-span-8 space-y-4">
								<div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
									<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
										<div>
											...
											<div className="mt-1 text-xs text-slate-500">
												Users, groups, cost centers,
												invoices, trips, purchases,
												vendors, RFQs, approvals.
											</div>
										</div>
										<div className="flex flex-wrap items-center gap-2">
											<Button
									...
									<div className="mt-3 rounded-2xl bg-slate-50 p-3 text-xs text-slate-600">
										Tip: use saved searches per role for
										common workflows. Use command center for
										actions.
									</div>
								</div>
								...
								<div className="space-y-4">
									{groupedResultCards.length ? (
										...
										<Empty
											title="No results"
											subtitle="Try a different query or enable more scopes."
										/>
									)}
								</div>
								...
								<div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
									<div className="flex items-start justify-between gap-3">
										<div>
											<div className="text-sm font-semibold text-slate-900">
												Premium: audit-friendly actions
											</div>
											<div className="mt-1 text-xs text-slate-500">
												Every destructive action
												requires a reason and produces
												an audit preview.
											</div>
										</div>
									...
									<div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
										<ExplainCard
											title="Reason"
											value="Why are you performing this action?"
										/>
										<ExplainCard
											title="Scope"
											value="Which org, group, or module is affected?"
										/>
										<ExplainCard
											title="Audit"
											value="Audit ID is created and visible to Admins."
										/>
									</div>
								</div>

								<div className="text-xs text-slate-500">
									C Global Search v2: unified search across
									all entities, premium saved searches per
									role, operator shortcuts, and audited quick
									actions.
								</div>
							</div>
						</div>
				...
				{detail ? (
					<div className="space-y-4">
						...
						<div className="rounded-3xl border border-slate-200 bg-white p-4">
							...
							<div className="mt-3 rounded-2xl bg-slate-50 p-3 text-xs text-slate-600">
								In production, this panel respects role
								permissions and redacts sensitive fields.
							</div>
						</div>
					</div>
				) : null}
					...
					<div className="lg:col-span-2 space-y-4">
						{actionKind === "rfq" ? (
							<div className="rounded-3xl border border-slate-200 bg-white p-4">
								...
								<div className="mt-3 rounded-2xl bg-amber-50 p-3 text-xs text-amber-900 ring-1 ring-amber-200">
									RFQs are routed to approvals and can use
									milestone payments after quote approval.
								</div>
							</div>
						) : null}
						...
						{actionKind === "budget" ? (
							<div className="rounded-3xl border border-slate-200 bg-white p-4">
								...
								<div className="mt-3 rounded-2xl bg-slate-50 p-3 text-xs text-slate-600">
									Premium: large budget changes can require
									approvals.
								</div>
							</div>
						) : null}
							...
							<div className="rounded-3xl border border-slate-200 bg-white p-4">
								...
								<div className="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
									...
									<SelectRow
										...
										options={[
											"CorporatePay only",
											"All services and purchases",
										]}
									/>
									<ToggleRow
										label="Notify users"
										value={freezeDraft.notifyUsers}
										onChange={(v) =>
											setFreezeDraft((p) => ({
												...p,
												notifyUsers: v,
								...
								<div className="mt-3 rounded-2xl bg-rose-50 p-3 text-xs text-rose-800 ring-1 ring-rose-200">
									Freeze is a sensitive action. Consider dual
									approval for full stop.
								</div>
							</div>
						) : null}
						...
						{actionKind === "export" ? (
							<div className="rounded-3xl border border-slate-200 bg-white p-4">
								...
								<div className="mt-3 rounded-2xl bg-slate-50 p-3 text-xs text-slate-600">
									Premium: scheduled exports can be configured
									in Integrations.
								</div>
							</div>
						) : null}
						...
						<div className="rounded-3xl border border-slate-200 bg-white p-4">
							...
							<div className="mt-2 text-xs text-slate-500">
								Reason is required for audit logs. Keep it
								specific.
							</div>
							<textarea
								value={reason}
							...
							<label className="mt-3 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
								...
								<div className="text-sm font-semibold text-slate-800">
									I confirm this action is authorized and will
									be audit logged.
								</div>
							</label>
							<div className="mt-2 text-xs text-slate-500">
								Minimum reason length: 8 characters.
							</div>
						</div>
					</div>
						...
						<div className="rounded-3xl border border-slate-200 bg-white p-4">
							<div className="flex items-start justify-between gap-3">
								<div>
									...
									<div className="mt-1 text-xs text-slate-500">
										This is the record that would be stored.
									</div>
								</div>
							...
							<div className="mt-3 rounded-2xl bg-slate-50 p-3 text-xs text-slate-600">
								Premium: map this to immutable audit log IDs and
							...
							<ul className="mt-3 space-y-2 text-sm text-slate-700">
								<li className="flex items-start gap-2">
									<span
										...
										style={{ background: EVZ.green }}
									/>{" "}
									Use group-level budgets before user caps.
								</li>
								<li className="flex items-start gap-2">
									<span
										className="mt-2 h-1.5 w-1.5 rounded-full"
									...
									/>{" "}
									For freezing, notify users to avoid
									disruptions.
								</li>
								<li className="flex items-start gap-2">
									<span
										...
										style={{ background: EVZ.green }}
									/>{" "}
									For RFQs, attach specs and required
									documents.
								</li>
							</ul>
						</div>
